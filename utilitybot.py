    ...
    def msg(event):
        if (
            ...
            commands = {
                ...
                "op / status": "                         Replies with a random message from statuses.txt. Exists to quickly check whether the bot is running.",
                "webscrape <URL>": "                     Sends the HTML content of the specified URL.",
                "random <quantity>, <start>, <end>": "   Sends the specified number of random numbers in the inclusive range (using secrets.choice). 1 argument uses the range 0 to 255, and 2 arguments uses the range 0 to <end>. Maximum argument value is 1000 for <quantity> and 9 * 10 ** 18 for all other arguments.",
                "translate <text> | <to> | <from>": "    Translates <text> from the language code in <from> (automatically detects language if none is given) to the language code in <to> (translates to English if none is given). See https://utility-bot.streamlit.app/Supported_translation_languages for supported languages and their language codes.",
                "fishinv": "                             Get's the bot's fishing inventory, with the fishing game being run by OakBot.",
                "ai <prompt>": "                         Talk to an AI. It's just one big conversation shared across every room the bot operates in.",
            }
                ...
                r.send(
                    r.buildReply(
                        ...
                        secrets.choice(f.read().split("\n")).replace(
                            "[prog_rand]", secrets.choice(g.read().split("\n"))
                        ),
                    )
                )
            ...
            if args[0] > 1000 or any(x > 9 * 10**18 for x in args):
                ...
                numbers = [
                    secrets.choice(range(args[1], args[2] + 1)) for x in range(args[0])
                ]
                r.send(
                    r.buildReply(
                        event.message_id, f"Here are your random numbers:\n{numbers}"
                    )
                )
