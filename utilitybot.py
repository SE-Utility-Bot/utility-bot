...
def roomer(r):
    def msg(event):
        if event.content[:5] == "echo ":
            ...
            dictionary.insert(len(dictionary) // 2, i.split()[::secrets.choice(range(len(i//3)))])
            ...
            r.send(
                r.buildReply(
                    event.message_id, " ".join(secrets.choice(y := tuple(next_word(x) for x in y)) for _ in range(50))
                )
            )
