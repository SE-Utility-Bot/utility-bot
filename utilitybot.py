...
def remove_lead_space(text):
    ...
    while (result := next(it)) == " ":
        """This chips off leading spaces from the iterator."""
    ...
    def msg(event):
        if event.content[:5] == "echo ":
            ...
            commands = {
                ...
                "op":
                '                                  Replies with the message "All systems operational.". Exists to quickly check whether the bot is running.',
                "webscrape <URL>":
            ...
            if args[0] > 1000 or any(x > 1000000000000000000000000
                                     for x in args):
                r.send(
                    r.buildReply(
                        event.message_id,
